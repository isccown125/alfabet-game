export class Component {
  ...
  setAttributes(...obj) {
    obj.forEach((el) => {
      if (!el.name) {
        throw new Error("Cannot set attributes without name");
      }
      if (typeof el.name !== "string") {
        throw new Error(
          `Object name or value must be a string. Given: name:${typeof obj.name}`
        );
      }
  ...
  setChild(...children) {
    ...
    children.forEach((el) => {
      ...
      if (options) {
        if (
          options.hasOwnProperty("prependChild") &&
          options.prependChild === true
        ) {
